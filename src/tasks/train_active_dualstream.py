    ...
    with torch.no_grad():
        for seq, stats_seq, prob, size, depth in loader:
            ...
            if mask.sum() > 0:
                ...
                loss_size = torch.tensor(0.0, device=device)
                loss_depth = torch.tensor(0.0, device=device)
            total_loss += (loss_prob + loss_size + loss_depth).item() * seq.size(0)
    return total_loss / max(1, len(loader.dataset))
...
def main():
    ...
    model = build_model("dualstream", seq_len=10).to(device)
    ...
    history = []
    best_val = float("inf")
    best_base_path = os.path.join(output_dir, "dualstream_3dcnn_lstm_base_refactor.pth")
    for epoch in range(args.epochs_base):
        ...
        print(f"[BASE] {epoch+1}/{args.epochs_base} train={train_loss:.4f} val={val_loss:.4f}")
    ...
    best_val = float("inf")
    best_active_path = os.path.join(output_dir, "dualstream_3dcnn_lstm_active_refactor.pth")
    for epoch in range(args.epochs_active):
        ...
        print(f"[ACTIVE] {epoch+1}/{args.epochs_active} train={train_loss:.4f} val={val_loss:.4f}")

    with open(os.path.join(output_dir, "train_active_dualstream_refactor_log.json"), "w", encoding="utf-8") as f:
