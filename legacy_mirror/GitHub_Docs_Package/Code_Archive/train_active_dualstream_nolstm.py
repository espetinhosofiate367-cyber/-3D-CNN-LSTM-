...
from dualstream_3dcnn_lstm import DualStream3DCNNNoLSTM
...
def select_uncertain_samples(model, loader, device, top_k=200):
    ...
    with torch.no_grad():
        for seq, stats_seq, path, end_row in loader:
            ...
            prob = torch.sigmoid(prob_pred).squeeze(1).cpu().numpy()
...
def main():
    ...
    model = DualStream3DCNNNoLSTM(seq_len=10).to(device)
    ...
    best_val = float('inf')
    best_path = os.path.join(output_dir, 'dualstream_3dcnn_nolstm_base.pth')
    ...
    if not os.path.exists(manual_labels):
        ...
        save_json(os.path.join(output_dir, 'active_learning_candidates_nolstm.json'), candidates)
    ...
    best_val = float('inf')
    best_path = os.path.join(output_dir, 'dualstream_3dcnn_nolstm_active.pth')
