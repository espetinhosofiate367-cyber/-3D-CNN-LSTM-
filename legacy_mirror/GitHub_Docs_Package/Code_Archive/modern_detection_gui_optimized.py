...
sys.path.append(os.path.join(project_root, '检测算法'))
sys.path.append(os.path.join(project_root, '核心程序'))
sys.path.append(os.path.join(project_root, '核心程序', '检测算法'))
...
class OptimizedDetectionGUI:
    def __init__(self, root):
        ...
        self.dl_model_path = os.path.join(os.path.dirname(__file__), 'deep_learning', 'active_learning_results', 'dualstream_3dcnn_lstm_active.pth')
        alt_model_candidates = [
            os.path.join(project_root, '核心程序', 'deep_learning', 'active_learning_results', 'dualstream_3dcnn_lstm_active.pth')
        ]
        for p in alt_model_candidates:
            if os.path.exists(p):
                self.dl_model_path = p
                break
        self.dl_model = None
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        
        try:
            # 加载新模型 (DualStreamModel)
            # 注意: final_model 需要 30 帧序列? 不，之前的 final_model 是 10 帧。
            # generate_final_visualizations.py 中 SEQ_LEN = 10。
            # realtime_detector.py 中是 30。
            # 这里的 final_model.pth 是 10 帧模型。
            self.seq_len = 10
            ...
            tried_paths = [p for p in [self.dl_model_path] + alt_model_candidates if os.path.exists(p)]
    ...
    def create_widgets(self):
        ...
        ttk.Label(ai_output_frame, textvariable=self.ai_depth_var).pack(side=tk.LEFT)
        ...
        try:
            ...
            if latest_frame is not None:
                ...
                if self.use_dl_detection and self.dl_var.get() and self.dl_model:
                    try:
                        self.frame_buffer.append(raw_matrix)
                        ...
                        if len(self.frame_buffer) == self.seq_len:
                            ...
                            with torch.no_grad():
                                ...
                                self.dl_probability = torch.sigmoid(prob).item()
    ...
    def scan_full_file(self):
        ...
        try:
            ...
            for i in range(total_frames):
                ...
                # Stats
                frame_stats = np.zeros((10, 3), dtype=np.float32)
                for j in range(10):
                    fr = seq_raw_reshaped[j]
                    frame_stats[j] = np.array([np.mean(fr), np.max(fr), np.std(fr)], dtype=np.float32)
                stats_tensor = torch.tensor(frame_stats).unsqueeze(0).to(self.device)
                ...
                with torch.no_grad():
                    prob, _, _ = self.dl_model(input_tensor, stats_tensor)
                    prob_val = torch.sigmoid(prob).item()
    ...
    def update_ai_output_labels(self):
        ...
        depth_cm = getattr(self, 'dl_pred_depth', None)

        self.ai_prob_var.set(f"结节概率: {prob:.2%}")
        ...
        if depth_cm is None:
            ...
            self.ai_depth_var.set(f"结节深度: {depth_cm:.2f} cm ({depth_cm * 10:.1f} mm)")
