...
def main():
    ...
    best_val = float('inf')
    best_path = os.path.join(output_dir, 'cnn3d_lstm_base.pth')
    ...
    if not os.path.exists(manual_labels):
        ...
        save_json(os.path.join(output_dir, 'active_learning_candidates_cnn3d_lstm.json'), candidates)
    ...
    best_val = float('inf')
    best_path = os.path.join(output_dir, 'cnn3d_lstm_active.pth')
