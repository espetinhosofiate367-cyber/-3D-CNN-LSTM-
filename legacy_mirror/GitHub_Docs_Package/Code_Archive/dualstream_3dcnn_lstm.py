...
class DualStream3DCNNLSTM(nn.Module):
    def __init__(self, seq_len=10, lstm_hidden=64, lstm_layers=1, dropout=0.3):
        super().__init__()
...
class CNN3DToLSTM(nn.Module):
    ...
    def forward(self, x, stats_seq=None):
        x = x.permute(0, 2, 1, 3, 4)
        ...
        x_shape = x_shape.mean(dim=[3, 4])
        x_shape = x_shape.permute(0, 2, 1)

        lstm_out, _ = self.lstm(x_shape)
        lstm_feat = torch.mean(lstm_out, dim=1)

        features = self.fusion_fc(lstm_feat)
...
class DualStream3DCNNNoLSTM(nn.Module):
    def __init__(self, seq_len=10, dropout=0.3):
        super().__init__()
        self.seq_len = seq_len
